...
st.header("CAP Synoptic Reporting: GIST - Resection (September 2023)")
...
with open("data/gist_sep2023.pkl", 'rb') as fp:
    data = pickle.load(fp)

if "protocol_content_gist" not in st.session_state:
    st.session_state.protocol_content_gist = {k:v for k, v in zip(data.keys(), [""] * len(data))}
    st.session_state.protocol_content_gist["Additional Findings"] = "" 

if "ln_counter_positive_gist" not in st.session_state:
    st.session_state.ln_counter_positive_gist = 0

if "ln_counter_negative_gist" not in st.session_state:
    st.session_state.ln_counter_negative_gist = 0

if "organ_pos_counter_gist" not in st.session_state:
    st.session_state.organ_pos_counter_gist = 0

if "organ_neg_counter_gist" not in st.session_state:
    st.session_state.organ_neg_counter_gist = 0
    ...
    with st.expander(section_title.replace("\t", "").upper()):
        ...
        if section["type"] == "dropdown" and isinstance(section["options"], list):
            ...
            if "Choose further" in section_line:
                ...
                st.session_state.ln_counter_negative_gist = lines_ln_neg
                if lines_ln_neg > 0:
                    section_line = section_line + "\n\tNegative for tumor:"

                for j in range(1, st.session_state.ln_counter_negative_gist+1):
        ...
        if section["type"] == "multiselect" and isinstance(section["options"], list):
            ...
            if "Specify individually" in section_line:
                ...
                st.session_state.organ_pos_counter_gist = organ_pos_num

                if organ_pos_num >0:
                    section_line = section_line + "\n\tPositive for tumor:"
            
                for k in range(1, st.session_state.organ_pos_counter_gist+1):
                ...
                st.session_state.organ_neg_counter_gist = organ_neg_num

                if organ_neg_num >0:
                    section_line = section_line + "\n\tNegative for tumor:"

                for l in range(1, st.session_state.organ_neg_counter_gist+1):
        ...
        with col1:
            if st.button("Add to Report", key = f"Add to report: {section_title}-{section_line}"):
                st.session_state.protocol_content_gist[section_title] = section_line.upper()
                
        with col2:
            if st.button("Remove from Report", key = f"Remove from report: {section_title}-{section_line}"):
                st.session_state.protocol_content_gist[section_title] = ""    

        with st.container():
            st.markdown("**Preview:**")
            st.text(f"{section_title}: {st.session_state.protocol_content_gist[section_title]}")
...
with st.expander("ADDITIONAL FINDINGS"):
    ...
    with col1:
        if st.button("Add to Report", key = "Additional findings - add"):
            st.session_state.protocol_content_gist["Additional Findings"] = additional_findings.upper()
    ...
    with st.container():
        st.markdown("**Preview:**")
        st.text(f"Additional Findings: {st.session_state.protocol_content_gist['Additional Findings']}")
...
final_report = "\n".join([f"{k}: {v}" for k, v in st.session_state.protocol_content_gist.items() if v !=""])
...
if clear_report:
    del st.session_state.protocol_content_gist
    st.rerun()