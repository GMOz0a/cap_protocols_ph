...
st.header("CAP Synoptic Reporting: Lung (September 2022)")
...
with open("data/lung_sep2022.pkl", 'rb') as fp:
    data = pickle.load(fp)

if "protocol_content_lung" not in st.session_state:
    st.session_state.protocol_content_lung = {k:v for k, v in zip(data.keys(), [""] * len(data))}
    st.session_state.protocol_content_lung["Additional Findings"] = "" 

if "ln_counter_positive_lung" not in st.session_state:
    st.session_state.ln_counter_positive_lung = 0

if "ln_counter_negative_lung" not in st.session_state:
    st.session_state.ln_counter_negative_lung = 0

if "organ_pos_counter_lung" not in st.session_state:
    st.session_state.organ_pos_counter_lung = 0

if "organ_neg_counter_lung" not in st.session_state:
    st.session_state.organ_neg_counter_lung = 0
    ...
    with st.expander(section_title.replace("\t", "").upper()):
        ...
        if section["type"] == "dropdown" and isinstance(section["options"], list):
            ...
            if "Choose further" in section_line:
                ...
                st.session_state.ln_counter_negative_lung = lines_ln_neg
                if lines_ln_neg > 0:
                    section_line = section_line + "\n\tNegative for tumor:"

                for j in range(1, st.session_state.ln_counter_negative_lung+1):
        ...
        if section["type"] == "multiselect" and isinstance(section["options"], list):
            ...
            if "Specify individually" in section_line:
                ...
                st.session_state.organ_pos_counter_lung = organ_pos_num

                if organ_pos_num >0:
                    section_line = section_line + "\n\tPositive for tumor:"
            
                for k in range(1, st.session_state.organ_pos_counter_lung+1):
                ...
                st.session_state.organ_neg_counter_lung = organ_neg_num

                if organ_neg_num >0:
                    section_line = section_line + "\n\tNegative for tumor:"

                for l in range(1, st.session_state.organ_neg_counter_lung+1):
        ...
        with col1:
            if st.button("Add to Report", key = f"Add to report: {section_title}-{section_line}"):
                st.session_state.protocol_content_lung[section_title] = section_line.upper()
                
        with col2:
            if st.button("Remove from Report", key = f"Remove from report: {section_title}-{section_line}"):
                st.session_state.protocol_content_lung[section_title] = ""    

        with st.container():
            st.markdown("**Preview:**")
            st.text(f"{section_title}: {st.session_state.protocol_content_lung[section_title]}")
...
with st.expander("ADDITIONAL FINDINGS"):
    ...
    with col1:
        if st.button("Add to Report", key = "Additional findings - add"):
            st.session_state.protocol_content_lung["Additional Findings"] = additional_findings.upper()
    ...
    with st.container():
        st.markdown("**Preview:**")
        st.text(f"Additional Findings: {st.session_state.protocol_content_lung['Additional Findings']}")
...
final_report = "\n".join([f"{k}: {v}" for k, v in st.session_state.protocol_content_lung.items() if v !=""])
...
if clear_report:
    del st.session_state.protocol_content_lung
    st.rerun()