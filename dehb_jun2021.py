...
st.header("CAP Synoptic Reporting: Distal Extrahepatic Biliary Ducts (June 2021)")
...
with open("data/dehb_jun2021.pkl", 'rb') as fp:
    data = pickle.load(fp)

if "protocol_content_dehb_jun2021" not in st.session_state:
    st.session_state.protocol_content_dehb_jun2021 = {k:v for k, v in zip(data.keys(), [""] * len(data))}
    st.session_state.protocol_content_dehb_jun2021["Additional Findings"] = "" 

if "ln_counter_positive_dehb_jun2021" not in st.session_state:
    st.session_state.ln_counter_positive_dehb_jun2021 = 0

if "ln_counter_negative_dehb_jun2021" not in st.session_state:
    st.session_state.ln_counter_negative_dehb_jun2021 = 0

if "organ_pos_counter_dehb_jun2021" not in st.session_state:
    st.session_state.organ_pos_counter_dehb_jun2021 = 0

if "organ_neg_counter_dehb_jun2021" not in st.session_state:
    st.session_state.organ_neg_counter_dehb_jun2021 = 0
    ...
    with st.expander(section_title.replace("\t", "").upper()):
        ...
        if section["type"] == "dropdown-distance" and isinstance(section["options"], list):
            ...
            if "Specify exact" in section_line:
                ...
                distance = st.text_input(f"Specify distance at least __ in {suffix}", key = f"{section_title}-{section_line}-at least distance")
        ...
        if section["type"] == "dropdown" and isinstance(section["options"], list):
            ...
            if "Choose further" in section_line:
                ...
                if subsection["type"] == "multiselect" and isinstance(subsection["options"], list):
                    ...
                    if len(additional_input) >=2:
                        ...
                        section_line = section_line.replace(" (Choose further)", "") + f"{additional_input} {subsection_suffix}".strip() + "."
                    else:
                        section_line = section_line.replace(" (Choose further)", "") + "."

            elif "Please specify further - subtype" in section_line:
                additional_input = st.text_input("Please specify the subtype", key = f"{section_title}-{section_line} + subtype")
            ...
            elif "Invades other structures" in section_line:
                section_line = ""
                additional_input = st.text_input("Input individual organ(s)/structures(s) involved by direct extension separated by a comma and a single white space (\", \")", key = f"{section}-{section_line}-invades-other-sections.")
                additional_input = additional_input.split(", ")

                if len(additional_input) >= 2:
                    section_line = "Invades the " + ", ".join(additional_input[:-1]) + f" and {additional_input[-1]}."
                elif (len) == 1:
                    section_line = "Invades the " + additional_input[0] + "."
                else:
                    section_line = "Invades other structures"
                ...
                st.session_state.ln_counter_positive_dehb_jun2021 = lines_ln_pos
                
                if lines_ln_pos >0:
                    section_line = section_line + "\n\tPositive for tumor:"
                st.caption("Input 1) number of positive lymph nodes; 2) total number of lymph nodes in the specimen; and 3) specimen label.")

                for i in range(1, st.session_state.ln_counter_positive_dehb_jun2021+1):
                ...
                st.session_state.ln_counter_negative_dehb_jun2021 = lines_ln_neg
                if lines_ln_neg > 0:
                    section_line = section_line + "\n\tNegative for tumor:"

                for j in range(1, st.session_state.ln_counter_negative_dehb_jun2021+1):
            ...
            elif "Equivocal (Explain)" in section_line:
                section_line = st.text_input("Equivocal due to:", key = f"{section_title}-{section_line}")
                section_line = "Equivocal due to "+ section_line + "."
        ...
        if section["type"] == "multiselect" and isinstance(section["options"], list):
            ...
            if "Specify individually" in section_line:
                ...
                st.session_state.organ_pos_counter_dehb_jun2021 = organ_pos_num

                if organ_pos_num >0:
                    section_line = section_line + "\n\tPositive for tumor:"
            
                for k in range(1, st.session_state.organ_pos_counter_dehb_jun2021+1):
                ...
                st.session_state.organ_neg_counter_dehb_jun2021 = organ_neg_num

                if organ_neg_num >0:
                    section_line = section_line + "\n\tNegative for tumor:"

                for l in range(1, st.session_state.organ_neg_counter_dehb_jun2021+1):
                ...
                for j, option in enumerate(section_line):
                    ...
                    elif "Specify percentage" in option:
                        end_index = option.find(" (")
                        option = option[:end_index]
                        additional_input = st.text_input(f"{option}: Specify exact percentage (%)", key = f"{section_title}-{section_line}-{option}-specify-percentage")
                        section_line_with_additional.append(f"{option} ({additional_input}%)")
                        additional_input = option
        ...
        with col1:
            if st.button("Add to Report", key = f"Add to report: {section_title}-{section_line}"):
                st.session_state.protocol_content_dehb_jun2021[section_title] = section_line.upper()
                
        with col2:
            if st.button("Remove from Report", key = f"Remove from report: {section_title}-{section_line}"):
                st.session_state.protocol_content_dehb_jun2021[section_title] = ""    

        with st.container():
            st.markdown("**Preview:**")
            st.text(f"{section_title}: {st.session_state.protocol_content_dehb_jun2021[section_title]}")
...
with st.expander("ADDITIONAL FINDINGS"):
    ...
    with col1:
        if st.button("Add to Report", key = "Additional findings - add"):
            st.session_state.protocol_content_dehb_jun2021["Additional Findings"] = additional_findings.upper()
    ...
    with st.container():
        st.markdown("**Preview:**")
        st.text(f"Additional Findings: {st.session_state.protocol_content_dehb_jun2021['Additional Findings']}")
...
final_report = "\n".join([f"{k}: {v}" for k, v in st.session_state.protocol_content_dehb_jun2021.items() if v !=""])
...
if clear_report:
    del st.session_state.protocol_content_dehb_jun2021
    st.rerun()