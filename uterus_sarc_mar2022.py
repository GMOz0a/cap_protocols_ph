...
st.header("CAP Synoptic Reporting: Uterus - Sarcoma March 2022")
...
with open("data/uterus_sarc_mar2023.pkl", 'rb') as fp:
    data = pickle.load(fp)

if "protocol_content_uterus_sarc" not in st.session_state:
    st.session_state.protocol_content_uterus_sarc = {k:v for k, v in zip(data.keys(), [""] * len(data))}
    st.session_state.protocol_content_uterus_sarc["Additional Findings"] = "" 

if "ln_counter_positive_uterus_sarc" not in st.session_state:
    st.session_state.ln_counter_positive_uterus_sarc = 0

if "ln_counter_negative_uterus_sarc" not in st.session_state:
    st.session_state.ln_counter_negative_uterus_sarc = 0

if "organ_pos_counter_uterus_sarc" not in st.session_state:
    st.session_state.organ_pos_counter_uterus_sarc = 0

if "organ_neg_counter_uterus_sarc" not in st.session_state:
    st.session_state.organ_neg_counter_uterus_sarc = 0
    ...
    with st.expander(section_title.replace("\t", "").upper()):
        ...
        if section["type"] == "dropdown-distance" and isinstance(section["options"], list):
            ...
            if section_line == "Specify exact distance":
                distance = st.text_input("Specify exact distance in millimeters (mm)", key = f"{section_title}-{section_line}-exact distance")
        ...
        if section["type"] == "dropdown" and isinstance(section["options"], list):
            ...
            if "Choose further" in section_line:
                ...
                st.session_state.ln_counter_positive_uterus_sarc = lines_ln_pos
                
                if lines_ln_pos >0:
                    section_line = section_line + "\n\tPositive for tumor:"
                st.caption("Input 1) number of positive lymph nodes; 2) total number of lymph nodes in the specimen; and 3) specimen label.")

                for i in range(1, st.session_state.ln_counter_positive_uterus_sarc+1):
                ...
                st.session_state.ln_counter_negative_uterus_sarc = lines_ln_neg
                if lines_ln_neg > 0:
                    section_line = section_line + "\n\tNegative for tumor:"

                for j in range(1, st.session_state.ln_counter_negative_uterus_sarc+1):
            ...
            elif "negative for tumor cells (Specify individually)"in section_line:
                lines_ln_neg = st.slider("Number of specimens with at least one (1) negative lymph node", 0, 25, 1, key = f"{section_title}-{section_line} slider negative")
        ...
        if section["type"] == "multiselect" and isinstance(section["options"], list):
            ...
            if "Specify individually" in section_line:
                ...
                st.session_state.organ_pos_counter_uterus_sarc = organ_pos_num

                if organ_pos_num >0:
                    section_line = section_line + "\n\tPositive for tumor:"
            
                for k in range(1, st.session_state.organ_pos_counter_uterus_sarc+1):
                ...
                st.session_state.organ_neg_counter_uterus_sarc = organ_neg_num

                if organ_neg_num >0:
                    section_line = section_line + "\n\tNegative for tumor:"

                for l in range(1, st.session_state.organ_neg_counter_uterus_sarc+1):
        ...
        with col1:
            if st.button("Add to Report", key = f"Add to report: {section_title}-{section_line}"):
                st.session_state.protocol_content_uterus_sarc[section_title] = section_line.upper()
                
        with col2:
            if st.button("Remove from Report", key = f"Remove from report: {section_title}-{section_line}"):
                st.session_state.protocol_content_uterus_sarc[section_title] = ""    

        with st.container():
            st.markdown("**Preview:**")
            st.text(f"{section_title}: {st.session_state.protocol_content_uterus_sarc[section_title]}")
...
with st.expander("ADDITIONAL FINDINGS"):
    ...
    with col1:
        if st.button("Add to Report", key = "Additional findings - add"):
            st.session_state.protocol_content_uterus_sarc["Additional Findings"] = additional_findings.upper()
    ...
    with st.container():
        st.markdown("**Preview:**")
        st.text(f"Additional Findings: {st.session_state.protocol_content_uterus_sarc['Additional Findings']}")
...
final_report = "\n".join([f"{k}: {v}." for k, v in st.session_state.protocol_content_uterus_sarc.items() if v !=""])
...
if clear_report:
    del st.session_state.protocol_content_uterus_sarc
    st.rerun()
