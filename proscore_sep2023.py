...
st.header("CAP Synoptic Reporting: Prostate - Core Needle Biopsy (September 2023)")
...
with open("data/proscore_sep2023.pkl", 'rb') as fp:
    data = pickle.load(fp)

if "protocol_content_proscore" not in st.session_state:
    st.session_state.protocol_content_proscore = {k:v for k, v in zip(data.keys(), [""] * len(data))}
    st.session_state.protocol_content_proscore["Additional Findings"] = "" 

if "ln_counter_positive_proscore" not in st.session_state:
    st.session_state.ln_counter_positive_proscore = 0

if "ln_counter_negative_proscore" not in st.session_state:
    st.session_state.ln_counter_negative_proscore = 0

if "organ_pos_counter_proscore" not in st.session_state:
    st.session_state.organ_pos_counter_proscore = 0

if "organ_neg_counter_proscore" not in st.session_state:
    st.session_state.organ_neg_counter_proscore = 0
    ...
    with st.expander(section_title.replace("\t", "").upper()):
        ...
        if section["type"] == "dropdown-count":
            ...
            if section_line == "Exact number":
                number_ln = st.text_input("Please specify exact number of tissue core(s)", key =  f"{section_title}-{i}-exact")
                section_line = f"{number_ln} {suffix}"+ "."

            elif section_line == "At least":
                number_ln = st.text_input("At least __ number of tissue core(s)", key =  f"{section_title}-{i}-atleast")
        ...
        if section["type"] == "dropdown-size":
            ...
            if section_line == "Specify exact size": 
                size_ln = st.text_input("Please specify exact number of tissue core(s)", key =  f"{section_title}-{i}-exact")
                section_line = f"{size_ln} {suffix}" + "."
            elif section_line == "Less than":
                size_ln = st.text_input("Please specify exact number of tissue core(s)", key =  f"{section_title}-{i}-at least")
                section_line = f"Less than {size_ln} {suffix}" + "."
        
            elif section_line == "Greater than":
                size_ln = st.text_input("Please specify exact number of tissue core(s)", key =  f"{section_title}-{i}-greater than")
        ...
        if section["type"] == "dropdown" and isinstance(section["options"], list):
            ...
            if "Choose further" in section_line:
                ...
                st.session_state.ln_counter_positive_proscore = lines_ln_pos
                
                if lines_ln_pos >0:
                    section_line = section_line + "\n\tPositive for tumor:"
                st.caption("Input 1) number of positive lymph nodes; 2) total number of lymph nodes in the specimen; and 3) specimen label.")

                for i in range(1, st.session_state.ln_counter_positive_proscore+1):
                ...
                st.session_state.ln_counter_negative_proscore = lines_ln_neg
                if lines_ln_neg > 0:
                    section_line = section_line + "\n\tNegative for tumor:"

                for j in range(1, st.session_state.ln_counter_negative_proscore+1):
                ...
                section_line = st.text_input("Specify exact percentage (%)", key = f"{section_title}-{section_line}")
                ...
                section_line = st.text_input("Equivocal because:", key = f"{section_title}-{section_line} + additonal text - equivocal")
                section_line = "Equivocal - "+ section_line + "."
        ...
        if section["type"] == "multiselect" and isinstance(section["options"], list):
            ...
            if "Specify individually" in section_line:
                ...
                st.session_state.organ_pos_counter_proscore = organ_pos_num

                if organ_pos_num >0:
                    section_line = section_line + "\n\tPositive for tumor:"
            
                for k in range(1, st.session_state.organ_pos_counter_proscore+1):
                ...
                st.session_state.organ_neg_counter_proscore = organ_neg_num

                if organ_neg_num >0:
                    section_line = section_line + "\n\tNegative for tumor:"

                for l in range(1, st.session_state.organ_neg_counter_proscore+1):
        ...
        if section["type"] == "list":
            ...
            if len(section_line) == 2:
                ...
                section_line = ", ".join(section_line[:-1]) + " and " + section_line[-1] + "."


        col1, col2 = st.columns([1,3])     
        with col1:
            if st.button("Add to Report", key = f"Add to report: {section_title}-{section_line}"):
                st.session_state.protocol_content_proscore[section_title] = section_line.upper()
                
        with col2:
            if st.button("Remove from Report", key = f"Remove from report: {section_title}-{section_line}"):
                st.session_state.protocol_content_proscore[section_title] = ""    

        with st.container():
            st.markdown("**Preview:**")
            st.text(f"{section_title}: {st.session_state.protocol_content_proscore[section_title]}")
...
with st.expander("ADDITIONAL FINDINGS"):
    ...
    with col1:
        if st.button("Add to Report", key = "Additional findings - add"):
            st.session_state.protocol_content_proscore["Additional Findings"] = additional_findings.upper()
    ...
    with st.container():
        st.markdown("**Preview:**")
        st.text(f"Additional Findings: {st.session_state.protocol_content_proscore['Additional Findings']}")
...
final_report = "\n".join([f"{k}: {v}" for k, v in st.session_state.protocol_content_proscore.items() if v !=""])
...
if clear_report:
    del st.session_state.protocol_content_proscore
    st.rerun()