...
st.header("CAP Synoptic Reporting: Colon and Rectum (June 2024)")
    ...
    with st.expander(section_title.replace("\t", "").upper()):
        ...
        with col1:
            if st.button("Add to Report", key = f"Add to report: {section_title}-{section_line}"):
                st.session_state.protocol_content_colonrectum_resection[section_title] = section_line.upper()
                
        with col2:
            if st.button("Remove from Report", key = f"Remove from report: {section_title}-{section_line}"):
                st.session_state.protocol_content_colonrectum_resection[section_title] = ""    

        with st.container():
            st.markdown("**Preview:**")
            st.text(f"{section_title}: {st.session_state.protocol_content_colonrectum_resection[section_title]}")
...
with st.expander("ADDITIONAL FINDINGS"):
    ...
    with col1:
        if st.button("Add to Report", key = "Additional findings - add"):
            st.session_state.protocol_content_colonrectum_resection["Additional Findings"] = additional_findings.upper()
    ...
    with st.container():
        st.markdown("**Preview:**")
        st.text(f"Additional Findings: {st.session_state.protocol_content_colonrectum_resection['Additional Findings']}")
...
final_report = "\n".join([f"{k}: {v}." for k, v in st.session_state.protocol_content_colonrectum_resection.items() if v !=""])
...
if clear_report:
    del st.session_state.protocol_content_colonrectum_resection
    st.rerun()
